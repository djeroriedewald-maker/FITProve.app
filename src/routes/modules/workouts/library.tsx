...
export default function WorkoutLibraryPage() {
      ...
      .then(async (data) => {
        ...
        try {
          ...
          const { data: blocks } = await supabase
            .from('workout_blocks')
            .select('workout_id, style, sequence')
            .in('workout_id', ids);
          const map = new Map<string, string>();
          const prefer = (s?: string | null) => {
            if (!s) return '';
            const t = String(s);
            if (/warm/i.test(t) || /cool/i.test(t)) return '';
            return t;
          };
          (blocks || []).forEach((b: any) => {
            const cur = map.get(b.workout_id);
            const pick = prefer(b.style) || cur || '';
            if (!cur && pick) map.set(b.workout_id, pick);
          });
          setStyleMap(map);
        } catch {}
      })
  ...
  const listFiltered = useMemo(() => {
    if (!workouts?.length) return [] as WorkoutT[];
    if (!wf.style || wf.style === 'any') return workouts;
    const want = String(wf.style).toLowerCase();
    return workouts.filter((w) => (styleMap.get(w.id) || '').toLowerCase() === want);
  ...
  return (
    <div className="px-4 py-4 max-w-3xl mx-auto">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <button className={baseField + " px-3 h-10"} onClick={() => setWf({ equipment: "any", duration: "any", style: "any" })} disabled={!hasFilters} title="Reset filters">
          Reset
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="mt-4 text-sm text-zinc-500 dark:text-zinc-400">Geen workouts gevonden. Pas je filters aan.</div>
      )}
    </div>
  );
}
