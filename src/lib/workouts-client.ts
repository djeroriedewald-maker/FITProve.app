...
/** Haal workout + blocks + oefeningen op (exercises via block_id i.p.v. workout_id). */
...
export async function listSessionSets(sessionId: string): Promise<SetLog[]> {
  const { data, error } = await supabase
    ...
    .order("exercise_id", { ascending: true }) // << alleen exercise_id
...
export async function addOrUpdateSet(payload: SetLogUpsert): Promise<any> {
  // Gebruik uitsluitend exercise_id; jouw set_logs heeft geen workout_exercise_id-kolom.
  const record: any = {
    session_id: payload.session_id,
    exercise_id: payload.exercise_id ?? payload.workout_exercise_id, // fallback indien caller de andere naam doorgeeft
  ...
  const { data, error } = await supabase
    .from("set_logs")
    .upsert(record, {
      onConflict: "session_id,exercise_id,set_index", // << conflict op exercise_id
    })
    .select("*");
